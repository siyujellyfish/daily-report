...
- Phase 6.7 — UI/UX refinement：完成並已透過 PR #14 squash merge 至 `main`；Production 與測試資料庫 schema 對齊，Production 無 synthetic/test fixture。
- Phase 7 — App Store limited-free weekly：排程與 Production transport 已完成設定；每週日於 Asia/Taipei 執行，第一輪 unattended Production 驗收等待 2026-09-27。Make 已切換 live delivery 為 schema v2，既有每日 Tasks 已同步 canonical category metadata；未建立 synthetic Production 報告。

## Documents

- `phase-7-app-store-weekly.md`：App Store 限時免費週報的資格規則、歷史去重、期限追蹤、schema v2 delivery contract 與第一輪 Production 驗收清單。
...
- `todo.md`：Phase 0–7 完整實作清單與驗收條件，作為主要進度來源。
