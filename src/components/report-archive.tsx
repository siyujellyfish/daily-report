...
export async function ReportArchive({ slug, page }: { slug: string; page: number }) {
	const result = await getReportsByCategory(slug, page);
	if (!result || page > result.pageCount) notFound();

	const { category } = result;
	const pageHref = (number: number) => number === 1
		? category.href
		: `${category.href}?page=${number}`;
	const pages = [...new Set([1, page - 1, page, page + 1, result.pageCount])]
		.filter((number) => number >= 1 && number <= result.pageCount)
		.sort((a, b) => a - b);

	return <div className={`category-scope ${category.style}`}>
		...
		<div className="archive-meta">
			<span>共 {result.total} 篇報告</span>
			<span>由新到舊{result.total > 0 && ` · 第 ${page} / ${result.pageCount} 頁`}</span>
		</div>
		{result.total === 0 ? <section className="empty">
			<h2>尚無已發布報告</h2>
			<p>報告發布後，將依日期顯示在這裡。</p>
			...
			{result.pageCount > 1 && <nav className="pagination" aria-label="報告列表分頁">
				{page > 1
					? <Link href={pageHref(page - 1)} prefetch={false} aria-label="上一頁">←</Link>
					: <span className="disabled" aria-disabled="true">←</span>}
				{pages.map((number, index) => <span className="page-group" key={number}>
					{index > 0 && number - pages[index - 1] > 1 && <span aria-hidden="true" className="ellipsis">…</span>}
					<Link
				...
				{page < result.pageCount
					? <Link href={pageHref(page + 1)} prefetch={false} aria-label="下一頁">→</Link>
					: <span className="disabled" aria-disabled="true">→</span>}
			</nav>}
		</>}
	</div>;
